...
||         Author:  Himanshu
||
||        Purpose:  This class has functions and operations pertaining to Queen Chess piece.
||
...
|+-----------------------------------------------------------------------
||
||   Constructors:  Queen(String PoString) 
||  				Initializes the Position of chess piece object
||					Assigns a memory to a Map. 
||
||  Class Methods:  checkQueenMove(int row,int column) returns boolean
 ...
 *
 */
public class Queen extends ChessSolver {
	...
	public boolean checkQueenMove(int row,int column) {
		if (row < 8 && row >= 0)
		{
	...
	/* check if the position occupied or not */
	...
	public Map<Integer,String> possibleMoves() {
		...
		int tempRow = row,tempColumn = column ;
			/* Calculating Possible move for a Queen */
				// Horizontal Move 
					for(int i = 0 ; i < 8 ; i++ )
					{
						tempColumn = i ;
						// Add to moves if valid and not occupied
						if (checkQueenMove(tempRow, tempColumn) && checkPositionOccupied(tempRow, tempColumn) && checkUniquePosition(tempRow, tempColumn) && checkCurrentPosition(tempRow, tempColumn)) {
					...
					while (checkQueenMove(tempRow, tempColumn)) {
						tempRow = tempRow + 1;
