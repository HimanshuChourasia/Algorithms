...
||         Author:  Himanshu
||
||        Purpose:  This class has functions and operations pertaining to Rook Chess piece.
||
...
|+-----------------------------------------------------------------------
||
||   Constructors:  Rook(String PoString) 
||  				Initializes the Position of chess piece object
||					Assigns a memory to a Map. 
||
||  Class Methods:  checkRookMove(int row,int column) returns boolean
 ...
 *
 */
public class Rook extends ChessSolver{
	String poString;
	int noOfPossibleMoves ;
	Map<Integer, String> moves ;
	...
	public boolean checkRookMove(int row,int column) {
		if (row < 8 && row >= 0)
		{
	...
	public Map<Integer,String> possibleMoves() {
		...
		int tempRow = row,tempColumn = column ;
			/* Calculating Possible move for a Rook */
			// Horizontal Move 
			for(int i = 0 ; i < 8 ; i++ )
			{
				tempColumn = i ;
				// Add to moves if valid and not occupied
				if (checkRookMove(tempRow, tempColumn) && checkPositionOccupied(tempRow, tempColumn) && checkUniquePosition(tempRow, tempColumn) && checkCurrentPosition(tempRow, tempColumn)) {
